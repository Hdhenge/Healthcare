...
import {
  Activity, LayoutDashboard, Map, TrendingUp,
  Users, Store, LogOut, Menu, ShieldCheck
} from 'lucide-react';

const navItems = [
  { to: '/admin', icon: LayoutDashboard, label: 'Master Dashboard', end: true },
  { to: '/admin/area-analytics', icon: Map, label: 'Area Analytics' },
  { to: '/admin/product-trends', icon: TrendingUp, label: 'Product Trends' },
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-gray-900/95 backdrop-blur-xl border-r border-white/10 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0`}>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all duration-200 ${
                  isActive
                    ? 'text-white bg-gradient-to-r from-violet-500/20 to-purple-500/20 border border-violet-500/30'
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 bg-gradient-to-br from-violet-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-sm">
